...
for ii = 0:nBodies-1
    ...
    if isMidlineBody
        ...
        bodyAttachName = ['R',bodyName(2:end)];
...
for ii = 0:nMuscles-1
    ...
    musclenewname = ['R',muscleoldname{1}(2:end)];
    ...
    for i = 0:nPP-1
        ...
        if any(strcmp(ppBody,midlineBodyList))
            ...
            ppNewBody = ['R',ppBody(2:end)];
        end
        ppLocMat = str2num(ppLoc(2:end-1));
        ppNewLocMat = ppLocMat.*mirror; % flips the sign of one of the values
        ppNewName = ppName;
        if strcmpi(ppName(1),'l')
            % checks for name of path point, renames to right
            ppNewName = ['r',ppName(2:end)];
        end
